...
## ⚠️ 执行期勘误（**T1–T7 实测发现，读本计划前先看这节**）

> 本节由 controller 在执行期追加。**计划正文的错误原样保留**——它是"当时怎么想的"的记录；
> 但**以本节为准**。每条都有实测事件，不是推测。
> 完整账在 `.superpowers/sdd/2026-09-16-vigil-m5-correctness/progress.md`（ledger）。
>
> 🛑 **E3/E4/E5 相关的整条「噪声过滤」线已于 2026-09-17 由用户裁定收束**——
> **不再用 LLM 判据批量清洗商业推广**，存量噪声改由人工手动删（overlay 软删 + 撤销）。
> **本计划里的 Task 9 Step 7/8/9（真库干跑 / apply / 复算判据）作废，不要执行。**
> 完整归档（**实测数字 / 为什么停 / 留下了什么 / 未测的缺口**）：
> **`docs/superpowers/specs/2026-09-17-vigil-filtering-archive.md`** ← **先读它**。

| # | 位置 | 原文是错的 | 实测真相 |
|---|---|---|---|
| **E1** | **Global Constraint 4 首句** | 「工作区**所有源码是 CRLF**」 | **假**。工作区是**混合**的（实测 99 `w/lf` / 27 `w/crlf`，且随任务漂移）。**唯一可信量具是 `git ls-files --eol`**。⛔ **`grep -c $'\r$'` 是静默假阴性**——对一个**全 CRLF** 文件返回 **0**；⛔ `od -c \| grep -c '\r'` 也不可靠（od 输出折行）。未跟踪的新文件用 `od -c` **用眼睛读** |
| **E2** | **Global Constraint 4 的变异纪律** | 「替换串里的换行写 `\r\n`」 | `git archive` 隔离副本**按构造是 LF** ⇒ 在副本里写 `\n`。**真正承重的是「替换后断言替换生效（锚点命中数 ≥1）」**，不是换行符本身 |
| **E3** | **Task 6 的注释（`:1481-1483`）** | 「非 redo 本来就不在集合里」 | **假命题**。`refine_runs` 是 `INSERT OR REPLACE`（`store.py:379-384`），`_record_error` 把 ok 覆写成 error ⇒ **非 redo 也会重抽墓碑消息、条目以新 item_id 复活**。**别照它把过滤挪进 redo 分支**（= 静默重开 D15）。已在 `refine.py` 改掉，`…even_without_redo` 钉着 |
| **E4** | **Task 9 的 `_dedupe_batch` 缺口（R3）** | `build_plan` 按 `counts[mid]` 算 `keep_n` | T9 的 `_produce()` **不调 `_dedupe_batch`** ⇒ 模型重抽同一消息两次时 `n_new=2` ⇒ 两条重复都留、**报「0 deleted」**。必须在算 `counts`/`judged` **之前**去重。**不许事后在 `to_delete` 里补一刀**（那还是两套判据） |
| **E5** | **Task 9 的 `repass()` LLM 失败分支** | `verdicts.setdefault(m.msg_id, 0)` | **批失败被当成「产出 0 条」⇒ 批量删条目**（数据丢失方向）。**最该拦的一条** |
| **E6** | **Task 9 与 overlay** | （未提） | `repass` **不读 overlay** ⇒ **用户软删的 item 会被它物理删掉**（D15 说"行仍在"，且此后 `undo` 救不回） |
| **E7** | **Task 8 的 `api_delete_item`（`:2392`）** | 只取 `srcs[0]` 落墓碑 | 而 `store.py:307` 的批内去重**来源取并集** ⇒ 一个 item 可有 ≥2 条源消息 ⇒ **只墓碑化第一条时 `--redo` 会复活它**（真库今天 0 例，T3 之后才出现）。**必须为每一条源消息都落** |
| **E8** | **Task 3 的 File Structure 行（`:51`）** | 说 `refine.py` 负责「批内去重调用」 | 实际在 **`save_items`**。**这行陈旧的表就是 E4 的成因**（`repass` 根本不调 `save_items`） |
| **E9** | **spec §3.3 的「5 处读路径」表** | 列了 5 处 | **漏了 `store.items_with_deadline`（`store.py:585`）**。按那张表自查 R13 会拿到**漏的清单**——而那张表正是用来防"漏一处没人红"的 |
| **E10** | **Task 6 Step 6 / Step 7** | Step 6 的 Expected 写 `assert calls == []`；Step 7 名为「阳性对照」 | Step 6 那个断言**在 brief 自己的测试里不存在**；Step 7 **实为第二条变异**（与"整段删"同族，两次都红排除不了"代码整体不存在"）。**真阳性对照要另跑** |
| **E11** | **Task 2 的 `_day_start_of`** | （未预见） | Windows 上对 **1970-01-03 之前**的本地时间抛 `OSError` ⇒ 挂在 `refine()` 每批的 try/except 上 ⇒ **整批 items 不落库**。已按逃逸舱修（纯算术回退）。⚠️ 真库有 **27 条 `ts=0`**，T9 会真实经过这条路径 |

**另两条执行期纪律（计划里没有，血泪换来的）**：

- **变异只在 `git archive` 副本里做**。实测事故：在工作区做变异时 agent 被中断 ⇒
  **变异体残留在工作区**，而**未跟踪文件的变异对 git 完全不可见** ⇒ 被误读成"干净的 TDD RED 态"。
- **RED 必须与 Expected 文本逐字对上**。字面量不符 ⇒ **先确认工作区干净再判 RED**
  （实测：那是"工作区带了实验品"唯一的外显信号）。
- **反着再问一句**：做完变异反证后必须问「有没有哪条约束是**变异全红但根本没被测到**的？」
  ——**六战六中**（T2 抓 1、T3 抓 3、T4 抓 6、T5 抓 3、T6 抓 4、T7 抓 4）。
  ⚠️ 反向也成立：**写得不好的变异体会造出「假红」**，而**假红比假绿更难察觉**（"变红了"看起来就是成功）。

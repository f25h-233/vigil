...
**错误体**：`400` / `404` 一律 `{"detail": "字符串"}`（FastAPI 默认形状；前端 `api.ts` 就是按它解析的）。

⚠️ **不是「一律」**（波 1 窄审查纠正的过度断言）：
- `422`（参数越界，如 `limit` 超上限）返回 `{"detail": [ {loc/msg/type}, ... ]}`——**列表**，不是字符串
- `503`（前端未构建）返回一段 **HTML**
- 前端的 `getJSON` 已用 `typeof detail === 'string'` 挡住这两种，**不会炸**，只是回落显示 `HTTP 422`。所以这是文档口径问题，**不需要改前端**。
...
- **`total` = 符合筛选条件的**总数**，与 `limit`/`offset` 无关**（不是当页条数）。
  ⚠️ 这条是波 1 窄审查抓出来的**契约空洞**：前端重度依赖它——`共 ${total} 条`、`items.length < total` 决定「加载更多」按钮出不出现、`还有 ${total - items.length} 条`。
  **若 T3 猜成「当页长度」，后果是分页按钮永不出现、总数永远显示 50**——静默降级、不报错，正是本项目反复防的那类假话。
- **`Category.count` = 该类目下的**条目**数**（前端直接显示为「N 条」）。
- **日期一律按本地时区**换算 epoch。⚠️ 不用 SQLite 的 `strftime('%s', ...)`——它按 **UTC** 解释同一个日期串，与产品口径差 8 小时（M2 实测记录在案）。
- **`/api/items` 排序为 `event_ts DESC, item_id DESC`（新的在前）**；`/api/digests/{id}` 里的 `items` 为 `event_ts ASC`（与日报正文同序）；**`/api/digests` 列表按 `window_from DESC`（最近的在前）**。
  三处顺序**各不相同是故意的**，不许「顺手改成一致」。
- **`q` 先 `strip()`**；空串等于不筛选。
...
> ⚠️⚠️ **本段已作废（2026-09-15，commit `cc6e2c0`）。以仓库里的 `web/src/views/Feed.tsx` 为准。**
>
> 下面这段是 T5 逐字照抄的版本（169 行）。**T5 的审查在它里面查出三条 Important 状态机缺陷**，已由修复轮修掉：
> ① 陈旧错误横幅常驻（`loadMore` 成功路径不清 `err`）；② 首屏失败无可见重试路径；③ **失败时 `setTotal(0)` —— 把「不知道」说成「有 0 条」**。
>
> 修复采用**结构性统一改法**而非三个补丁：`items`/`total`/`kind`/`q` 合并成一份 `shown: {…} | null`（数据与「它属于哪个查询」同生同死），错误只在请求失败时写入、在任何请求开始时清除，并给横幅加了显式「重试」。
>
> **谁再照抄下面这段，就会把三条缺陷原样带回来。** 保留它只是为了让「附录源码也会带缺陷」这件事有据可查——这是本里程碑出现**两次**的模式（T4 审查也在附录里查出 3 条）。审查者的原话：**「真装真构建通过」只能证明类型与打包，证明不了状态机**。
